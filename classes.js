...
class Environment {
  ...
  update() {
    ...
    for (let fairy of this.fairies) {
      ...
      fairy.update(this.guide, this.blackHole, this.oasis);
      if (fairy.saved) {
        this.savedFairies++;
...
class Oasis {
  ...
  saveFairy(fairy) {
    ...
    fairy.rad = this.size
    fairy.ogRad = fairy.rad;
    fairy.saved = true;
    fairy.guided = false;
    fairy.danceTimer = new Timer(8);
  }
